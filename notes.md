...
## Dev log — Retention pass (onboarding, reward pacing, progression) — 2026-08-23

**Why players leave (diagnosis from this repo's own QA data).** Onboarding: the first obstacle arrives ~7s in with zero teaching, mobile players get no control hint at all (`#controlHint` is display:none on coarse pointers — QA issue C2), and death gives no feedback on what killed you (C4), so new players can't learn. Reward pacing: the first milestone was at 500m — past where most casual runs die; Golden Kicks first appeared past 220m so most first runs never saw the only power-up (A4); milestone bonuses dwarfed skill income late-game (B3). Progression: the only persistent stat was best score — coins reset every run, so collecting them was pointless across sessions, and nothing pulled players back for "one more run".

**Fixes.**
- **Onboarding (teach at the moment of need):** first-run detection via lifetime run count. During the first run ever, the first obstacle of each kind carries a one-shot tutorial flag; when it approaches (z < 42) a telegraphed prompt fires — `JUMP! ↑` / `SLIDE! ↓` / `MOVE! ← →`. Mobile first run gets a swipe-gesture hint (`#touchHint`, coarse-pointer only, 6s). Game-over now shows a death recap naming the killer and its counter-move ("You hit a hurdle — JUMP over these…"), defaulting to a generic controls line. Closes C2 and C4.
- **Reward pacing:** milestone ladder front-loaded to 150m / 300m, then every 500m, bonuses escalating 100→200→300…; Golden Kicks eligibility 220m → 120m (A4); coin/near-miss/smash income now scales ×(1+difficulty) — +15→+30, +25→+50, +50→+100 at max difficulty — so late-game skill income stops being dwarfed by distance bonuses (B3, and softens the B2 cliff with more frequent early dopamine).
- **Progression (the return loop):** new `js/meta.js` (MetaSys) persists lifetime runs, coin bank, XP and achievements (`sp-runs`, `sp-coinbank`, `sp-xp`, `sp-ach`; `sp-best` unchanged). XP = score/10 per run; level curve threshold 250·L·(L−1) (LV2 at 500 XP, LV3 at 1500…). 8 achievements (first run, 500m, 1000m, 100 lifetime coins, Golden Kicks, 10 runs, 10k score, LV5); `kicks` unlocks mid-run with a canvas toast, the rest at recordRun. Title screen shows LV / coin bank / achievement chips; game-over shows the XP progress bar (the "almost leveled up" pull), coin bank line, and unlocked-achievement line.

**TDD.** 12 new Playwright checks appended to `tests/qa.mjs` (section 6) and watched RED first (fresh-profile state, touch hint, tutorial popup, death recap text, persisted bank/xp/runs/achievement, XP bar render, returning-player state, title chips, 150m milestone, kicks-from-120m, +30 max-difficulty coin) — then green after implementation.

**Reviewer pass caught one blocker.** `sp-ach` containing valid-but-non-array JSON (`'5'`, `'null'`) passed the JSON.parse try/catch and then threw on `.filter` *outside* it → permanent boot crash loop (worst-case failure mode, invisible to the suite's fresh contexts). Reproduced red standalone (`ach.filter is not a function`), fixed with an `Array.isArray` guard, and added a permanent regression check (corrupt `sp-ach`/`sp-xp`/`sp-coinbank` values must not brick boot).

**Verification (Playwright).** `npm test`: ALL GREEN, 38 checks (25 pre-existing + 12 retention + 1 corruption regression), FPS floors unaffected (mobile 58.3, desktop 20.7 — no measurable cost from the meta layer). `FAIRNESS_RUNS=5 npm run test:fairness`: 5/5 perfect-bot survival at max difficulty (dist 2032–2086m) — spawn cadence untouched except the sneaker threshold. No console errors on any viewport. Evidence: `evidence/retention-gameover.jpg` (recap + XP bar + achievements on first death), `evidence/retention-title.jpg` (returning-player title with LV/bank/achievements).
